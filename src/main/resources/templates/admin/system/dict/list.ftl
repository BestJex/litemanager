...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${basePath!}/admin/system/dict/list',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑字典",
                    type : 2,
                    content : "${basePath!}/admin/system/dict/edit?id="+data.id,
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该字典么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${basePath!}/admin/system/dict/deleteById",{"id":data.id},function (res){
                                if(res.success){
            ...
            if(obj.event === "addType"){
                var addTypeIndex = layer.open({
                    title : "新增TYPE的值",
                    type : 2,
                    content : "${basePath!}/admin/system/dict/add?type="+data.type,
                    success : function(layero, index){
                        setTimeout(function(){
                            layer.tips('点击此处返回字典列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
        ...
        var active={
            addUser : function(){
                var addIndex = layer.open({
                    title : "添加字典",
                    type : 2,
                    content : "${basePath!}/admin/system/dict/add",
        ...
        form.on("submit(changeType)",function (data) {
            ...
            $.post("${basePath!}/admin/system/dict/editType",data.field,function(res){
                if(!res.success){
                    layer.msg(res.message);
                }else{
