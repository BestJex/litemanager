...
<head>
    ...
    <link rel="stylesheet" href="${basePath!}/static/formatJSON/jsonFormater.css" media="all" />
    <style>
        #Canvas {
            margin-top: 13px;
            padding: 20px 20px;
...
<script type="text/javascript" src="${basePath!}/static/formatJSON/jsonFormater.js?v=3.0"></script>
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${basePath!}/admin/system/log/list',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该日志么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${basePath!}/admin/system/log/delete",{"ids":[data.id]},function (res){
                           if(res.success){
                ...
                if(data.length > 0){
                    layer.confirm("你确定要删除这些日志么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            ...
                            $.post("${basePath!}/admin/system/log/delete",{ids:d},function (res) {
                                if(res.success){
                                    layer.msg("删除成功",{time: 1000},function(){
                                        table.reload('test', t);
                                    });
