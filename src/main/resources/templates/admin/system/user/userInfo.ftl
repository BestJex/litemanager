...
<head>
    ...
    <link rel="stylesheet" href="${basePath!}/static/css/user.css?v=2.0" media="all" />
</head>
...
<form class="layui-form">
    ...
    <div class="user_right">
        ...
        <button type="button" class="layui-btn layui-btn-normal" id="test1"><i class="layui-icon"></i>上传头像</button>
        <img <#if (userinfo.icon??)>src="${userinfo.icon}" <#else> src="${basePath!}/static/images/face.jpg " </#if> class="layui-circle" id="userFace">
    </div>
...
<script>
    layui.use(['form','jquery','layer','upload','laydate'],function() {
        ...
        form.on("submit(changeUser)",function (data) {
            ...
            $.post("${basePath!}/admin/system/user/saveUserinfo",data.field,function(res){
