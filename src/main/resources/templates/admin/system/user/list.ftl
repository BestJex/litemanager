...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${basePath!}/admin/system/user/list',
            method:'post',
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑用户",
                    type : 2,
                    content : "${basePath!}/admin/system/user/edit?id="+data.id,
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该用户么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        $.post("${basePath!}/admin/system/user/delete",{"id":data.id},function (res){
                           if(res.success){
        ...
        var active={
            addUser : function(){
                var addIndex = layer.open({
                    title : "添加会员",
                    type : 2,
                    content : "${basePath!}/admin/system/user/add",
                ...
                if(data.length > 0){
                    ...
                    layer.confirm("你确定要删除这些用户么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            ...
                            $.ajax({
                                type:"POST",
                                url:"${basePath!}/admin/system/user/deleteSome",
                                dataType:"json",
