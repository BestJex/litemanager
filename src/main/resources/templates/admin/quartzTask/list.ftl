...
<script>
    layui.use(['layer','form','table','laydate'], function() {
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑定时任务",
                    type : 2,
                    content : "${basePath!}/admin/quartzTask/edit?id="+data.id,
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该定时任务么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${basePath!}/admin/quartzTask/delete",{"ids":[data.id]},function (res){
                                if(res.success){
            ...
            if(obj.event === "paush"){
                layer.confirm("你确定要暂停该定时任务么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${basePath!}/admin/quartzTask/paush",{"ids":[data.id]},function (res){
                                if(res.success){
            ...
            if(obj.event === "resume"){
                layer.confirm("你确定要恢复该定时任务么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${basePath!}/admin/quartzTask/resume",{"ids":[data.id]},function (res){
                                if(res.success){
            ...
            if(obj.event === "run"){
                layer.confirm("你确定要立即执行该定时任务么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${basePath!}/admin/quartzTask/run",{"ids":[data.id]},function (res){
                                if(res.success){
        ...
        var t = {
            elem: '#test',
            url:'${basePath!}/admin/quartzTask/list',
            method:'post',
        ...
        var active={
            addQuartzTask : function(){
                var addIndex = layer.open({
                    title : "添加定时任务",
                    type : 2,
                    content : "${basePath!}/admin/quartzTask/add",
